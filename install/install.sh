...
create_venv(){
  ...
  $VENV_PATH/bin/python -m pip install --upgrade pip setuptools wheel --retries 1 --default-timeout=300 > /dev/null
  $VENV_PATH/bin/python -m pip install -r $PIP_REQUIREMENTS_FILE --retries 1 --default-timeout=300 > /dev/null &
  ...
  if [[ -n "$WS_TYPE" ]]; then
    ...
    $VENV_PATH/bin/python -m pip install -r $WS_REQUIREMENTS_FILE --retries 1 --default-timeout=300 > ws_pip_install.log &
